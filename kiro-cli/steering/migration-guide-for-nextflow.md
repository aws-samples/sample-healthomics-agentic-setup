...
---
name: migration-guide-for-nextflow
description: Migrating existing Nextflow workflows to AWS HealthOmics
---
...
**Objective**: Verify all processes have CPU and memory declarations.
...
4. IF creating new S3 buckets, configure them with:
...
5. Create and run a script (`scripts/migrate_references_to_s3.sh`) to retrieve all required files and copy them to the S3 bucket structure.
6. Update sample sheets to point to new S3 URIs then copy those sample sheets to S3.
7. Update any hardcoded paths in the workflow definition or config files with the new S3 URLs.
8. Update the inventory CSV with the S3 URIs of all relocated files
